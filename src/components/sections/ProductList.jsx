...
export default function ProductList({ selectedCategory, setSelectedCategory, addToCart }) {
  const { token } = useContext(AuthContext)
  const navigate = useNavigate()
  const filteredProducts = selectedCategory === "Hamısı" ? initialProducts : initialProducts.filter(p => p.category === selectedCategory)

  const handleAddToCart = (product) => {
    if (!token) {
      alert("Məhsulu səbətə əlavə etmək üçün qeydiyyatdan keçməlisiniz!")
      navigate('/login')
      return
    }
    addToCart(product)
  ...
  return (
    <section id="products" className="max-w-7xl mx-auto px-6 py-8">
      ...
      {filteredProducts.length === 0 ? ( <NotFound resetFilter={() => setSelectedCategory("Hamısı")} />
      ) : 
